      ...
      <div className="mx-auto w-full max-w-[1400px] px-5 py-16 sm:px-8 sm:py-20 lg:px-12">
        ...
        <div className="grid gap-12 border-b border-white/10 pb-14 lg:grid-cols-12 lg:gap-16">
          ...
          <div className="lg:col-span-5 lg:flex lg:items-center lg:justify-end">
            <a
              ...
              className="group inline-flex w-full items-center justify-between gap-4 rounded-2xl border border-white/15 bg-white/[0.03] p-6 transition-all duration-300 hover:border-gold/40 hover:bg-white/[0.06] lg:w-auto"
            >
        ...
        <div className="grid gap-12 py-14 lg:grid-cols-12">
          <div className="lg:col-span-5">
            ...
            <p className="mt-5 font-mono text-xs uppercase tracking-[0.28em] text-gold">
              {BRAND.tagline}
            </p>
            ...
            <ul className="grid grid-cols-2 gap-x-4 gap-y-4 sm:grid-cols-1">
              {NAV_ITEMS.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="text-base text-white/60 transition-colors hover:text-gold"
                  >
                    {item.label}
                  </Link>
                </li>
            ...
            <ul className="space-y-4 text-base">
              <li>
                ...
                >
                  ...
                  <span>
                    <span className="block font-mono text-xs uppercase tracking-[0.24em] text-white/45">
                      WhatsApp
                    </span>
                    {BRAND.phoneDisplay}
                  </span>
...
function FooterHeading({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="mb-6 font-mono text-xs font-medium uppercase tracking-[0.28em] text-gold">
      {children}
    </h3>
  );
}
