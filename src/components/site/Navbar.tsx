...
export function Navbar() {
  ...
  return (
    <>
      ...
      >
        <nav className="mx-auto flex h-16 w-full max-w-[1400px] items-center justify-between px-6 sm:h-[72px] sm:px-10 lg:px-16">
          {/* Logo */}
          <Link href="/" aria-label="Aldeora Creative — Home" className="flex items-center hover:opacity-80 transition-opacity">
            <Logo />
          </Link>
              ...
              <li key={item.href}>
                <Link
                  ...
                  className={cn(
                    ...
                    isActive(item.href) ? "text-ink" : "text-ink/50 hover:text-ink"
                  )}
                >
                  {item.label}
                  {isActive(item.href) && (
          ...
          <div className="hidden lg:block">
            <a
              ...
              className="btn-shine group inline-flex h-11 items-center gap-2 rounded-full bg-ink px-6 text-xs font-semibold uppercase tracking-[0.14em] text-white transition-all duration-400 hover:bg-gold hover:text-ink"
            >
      ...
      {open && (
        ...
        >
          <div className="flex h-16 shrink-0 items-center justify-between border-b border-white/10 px-6 sm:h-[72px] sm:px-10">
            <Logo tone="light" />
            <button
          ...
          <nav className="flex-1 overflow-y-auto px-6 py-8 sm:px-10">
              ...
              {NAV_ITEMS.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={cn(
                      "flex items-center justify-between border-b border-white/10 py-6 font-display text-3xl font-light tracking-tight transition-colors",
          ...
          <div className="shrink-0 border-t border-white/10 p-6 sm:p-10">
            <a
              href={WA_DEFAULT}
              target="_blank"
              rel="noopener noreferrer"
            ...
            <p className="mt-4 text-center text-sm text-white/50">
              Or email{" "}
              <a href="mailto:aldeoracreative@gmail.com" className="text-gold">aldeoracreative@gmail.com</a>
            </p>
          </div>
        </div>
      )}
